...
load("~/final.RData")
```
{#css_id} Executive summary

This document is a report of the creation of a movie recommendation algorithm developed in R, and is one of the two final projects for HarvardX Data Science Professional Certificate (PH 125x program).
...
The full table is divided into 3 subparts. The first and second parts are the 'train set' and the 'test set' - and they form what I call the "Usable set" -. They are respectively gathering 7.2 and 1.8 million entries and will both be extensively used to 'train' (find prediction patterns) and 'test' my algorithm (compare 'trained' predictions with observed 'test' data).
The third subpart, the 'validation' set, is composed of the remainder of the data (1 million entries) and will be used only once when eventually measuring the precision of my final algorithm. Indeed, for evaluation purposes, such a set is required in order to put my algorithm to work in a "one-shot" application, with data/information it hasn't encountered yet.


The "Usable set" (7.2 + 1.8 = 9 Million) comprehends ratings given by 69878
users about 10677 movies. Ratings range from 0.5 to 5 stars, with a mean of 3.51 stars overall. This mean is the first and most basic estimator of the model.

The different cleaning operations aren't numerous, because the database has already been very properly created by the movielens group. Still, some work had to be done in view of being assured that every movie, user, movie age, and genre mixture are present in both the train_set and test_set when comparing for prediction. The same applies between the train_set and the validation_set. When doing this comparison, observations present in only one of the two sets were deleted.
...
For example, after having tried to include a date effect, the RMSE obtained was inconclusive, so I decided to delete the 'timestamp' variable from train and test sets to make things lighter. Still, at the very end of the script, you can see the code referring to the operations on the date variable, and to other abandonned calculations.
...
The userId, movieId, and genres variables are used exactly the same way. I am regrouping observations as follows, and compute a regularized average. Note that this regularization process (inclusion of lambda in the calculation of the mean) is made to penalize estimators that were computed thanks to a relatively small number of observations.
...
'bi_final' captures the difference in the average rating of movie i compared to the overall average rating. Because there are 10641 different movies in the dataset, there are 10641 different bi_final. 'y' is the regularization term, which value's determination will be explained a bit later. 
...
I now have four models, and a computed RMSE for each of them.
...
What is important to notice in this complicated chunk of code is the computation of 'bi_final' and 'bu_final'. I had a 'y' term to the calculus of the average, which penalizes movies that have been rated only a very few number of times, by shrinking their average a bit more than observed. It is not 'fair' that a movie rated 5 stars but only appearing twice in the database, has the same impact in the model than a movie rated 5 stars a hundred times. Same goes for users. 
...
I have computed a total of 7 models and their corresponding RMSEs.
...
> rmse_results %>% knitr::kable()
```
With a RMSE of 0.864827, the final model can be considered quite good even if some improvements could be made. For example, maybe there is a movie which holds a unique age, and I did not regularized the age_rating. Moreover, after having finished the model as it is right now, it could be possible to use the test_set as a supplement to the training_set, maybe gaining a little explanation power when testing it on the final Validation set.
